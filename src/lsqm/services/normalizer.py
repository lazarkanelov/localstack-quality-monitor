...
def cloudformation_to_terraform(cf_template: str, logger: logging.Logger | None = None) -> tuple[dict[str, str], set[str]]:
...
def serverless_to_terraform(serverless_yml: str, logger: logging.Logger | None = None) -> tuple[dict[str, str], set[str]]:
    ...
    for func_name, func_config in functions.items():
        ...
        runtime = func_config.get("runtime", config.get("provider", {}).get("runtime", "python3.11"))
...
def _add_api_gateway_event(parts: list[str], func_name: str, http_config: dict | str, services: set[str]) -> None:
...
def _add_sqs_event(parts: list[str], func_name: str, sqs_config: dict | str, services: set[str]) -> None:
...
def _add_schedule_event(parts: list[str], func_name: str, schedule_config: dict | str, services: set[str]) -> None:
